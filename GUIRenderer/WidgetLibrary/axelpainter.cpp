...
/*! \class AxelPainter
  ...
  restore. So while it is possible to pass a AxelPainter instance to a function that expects a
  QPainter pointer, some of the workarounds and tweaks will be unavailable to the function (because
  it will call the base class implementations of the functions actually hidden by AxelPainter).
*/

/*!
  Creates a new AxelPainter instance and sets default values
*/
AxelPainter::AxelPainter() :
  QPainter(),
  mModes(pmDefault),
...
/*!
  Creates a new AxelPainter instance on the specified paint \a device and sets default values. Just
...
*/
AxelPainter::AxelPainter(QPaintDevice *device) :
  QPainter(device),
  mModes(pmDefault),
  mIsAntialiasing(false)
{
...
*/
void AxelPainter::setPen(const QPen &pen)
{
  QPainter::setPen(pen);
...
/*! \overload
  ...
  \note this function hides the non-virtual base class implementation.
*/
void AxelPainter::setPen(const QColor &color)
{
...
/*! \overload
  ...
  \note this function hides the non-virtual base class implementation.
*/
void AxelPainter::setPen(Qt::PenStyle penStyle)
{
...
/*! \overload
  ...
  \note this function hides the non-virtual base class implementation.
*/
void AxelPainter::drawLine(const QLineF &line)
{
...
/*!
  ...
  with QPainter::Antialiasing directly, as it allows AxelPainter to regain pixel exactness between
  antialiased and non-antialiased painting (Since Qt < 5.0 uses slightly different coordinate systems for
  AA/Non-AA painting).
*/
void AxelPainter::setAntialiasing(bool enabled)
{
...
*/
void AxelPainter::setModes(AxelPainter::PainterModes modes)
{
  mModes = modes;
}
...
/*!
  ...
  The Constructor \ref AxelPainter(QPaintDevice *device) which directly starts painting also sets
  the render hint as appropriate.

  \note this function hides the non-virtual base class implementation.
*/
bool AxelPainter::begin(QPaintDevice *device)
{
...
/*! \overload
  ...
  fixes/workarounds optimized for certain output devices.
*/
void AxelPainter::setMode(AxelPainter::PainterMode mode, bool enabled)
{
...
/*!
  Saves the painter (see QPainter::save). Since AxelPainter adds some new internal state to
...
/*!
  Restores the painter (see QPainter::restore). Since AxelPainter adds some new internal state to
  QPainter, the save/restore functions are reimplemented to also save/restore those members.
...
*/
void AxelPainter::makeNonCosmetic()
{
